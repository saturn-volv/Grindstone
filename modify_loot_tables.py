...
# Eventually I'll add a new system that uses sysargs to allow for more customization rather than just, dumping code manually.
...
item_cache = {}
def get_item_json(path : str) -> object:
    try:
        ...
        with open(path, "r") as f:
            ...
            data = (text, json_data)
            item_cache[path] = data
            return data

def apply_modifier(path:str, appended_data:object, predicate) -> None:
    for filedir in glob.glob('input\\*.json'):
        ...
        if result:
            try:
                json_data : object = get_item_json(filedir)[1]
